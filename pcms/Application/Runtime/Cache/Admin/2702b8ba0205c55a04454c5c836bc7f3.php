...
<tr><!-- 添加一级项目 -->
	...
	<div class="col-md-3 col-sm-3" style="padding-top:15px">
		<select name="categoryId" class="form-control input-sm">  
			...
			<?php if(is_array($categoryTree)): $i = 0; $__LIST__ = $categoryTree;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$category): $mod = ($i % 2 );++$i;?><option value="<?php echo ($category["id"]); ?>#category"><#if category.grade != 0><#list 1..category.grade as i>&nbsp;&nbsp;</#list></#if>${category.name}</option><?php endforeach; endif; else: echo "" ;endif; ?>
		</select>
	</div>
